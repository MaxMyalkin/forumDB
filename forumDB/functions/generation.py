...
##    (int(randrange(1, 10)), "email" + format(randrange(1, 100))))
#
#db.commit()
#cursor.close()
#db.close()
...
def insert_user(args):
	c = db.cursor()
	try:
		c.execute(u"""INSERT INTO Users (id, username, name, email, about, isAnonymous)
					 VALUES (%s, %s, %s, %s, %s, %s)""",
...
def insert_forum(args):
	c = db.cursor()
	try:
		c.execute(u"""INSERT INTO Forums (id, name, short_name, user)
					 VALUES (%s, %s, %s, %s)""",
				  (args['id'], args['name'], args['short_name'], args['user']))
		db.commit()
	except Exception as e:
		db.rollback()
		raise e
...
def insert_thread(args):
	c = db.cursor()
	try:
		c.execute(u"""INSERT INTO Threads (id, forum, title, isClosed, user, user_id, date, message, slug, isDeleted)
					 VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)""",
				  (args['id'], args['forum'], args['title'], int(args['isClosed']), args['user'],
...
def insert_post(args):
	c = db.cursor()
	try:
		c.execute(u"""INSERT INTO Posts (id, date, thread, message, user, user_id, forum, parent,
		...
		c.execute(u"""UPDATE Threads SET posts = posts + 1
